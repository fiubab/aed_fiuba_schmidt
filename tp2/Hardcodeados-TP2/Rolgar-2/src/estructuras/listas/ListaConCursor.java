package src.estructuras.listas;
...
public class ListaConCursor<T extends Object> {
	...
	public void agregar(int posicion, T elemento) throws Exception {
		...
		if (posicion == 1) {
			...
			NodoSimplementeEnlazado<T> anterior = this.getNodo(posicion -1);
	...
	public void remover(int posicion) throws Exception {
		validarPosicion(posicion);
		this.cursor = null;
		NodoSimplementeEnlazado<T> removido = null;
		if (posicion == 1) {
	...
	public void remover(T valor) throws Exception {
		NodoSimplementeEnlazado<T> cursor = this.primero;
		int posicion = 1;
		while (cursor != null) {
	 ...
	 */
	private NodoSimplementeEnlazado<T> getNodo(int posicion) throws Exception {
		validarPosicion(posicion);
	...
	//Solucion 2
//	public boolean contieneS2(String valor) throws Exception {
//		if (valor == null) {
//			throw new Exception("El valor no puede ser nulo");
//		}
//		NodoSimplementeEnlazado<T> cursor = this.primero;
//		while (cursor != null) {
//			Participante participante = (Participante) cursor.getDato();
//			if (participante.getNombre().equals(valor)) {
//				return true;
//			}
//			cursor = cursor.getSiguiente();
//		}
...
//	}
	...
	//Solucion 3
	public boolean contieneS3(Object valor) throws Exception {
		if (valor == null) {
			throw new Exception("El valor no puede ser nulo");
		}
		NodoSimplementeEnlazado<T> cursor = this.primero;
	...
	//Solucion 4
	public boolean contieneS4(Object valor) throws Exception {
		if (valor == null) {
			throw new Exception("El valor no puede ser nulo");
		}
		NodoSimplementeEnlazado<T> cursor = this.primero;
		while (cursor != null) {
			if (cursor.getDato().toString().equals(valor.toString())) {
				return true;
			}
			cursor = cursor.getSiguiente();
		}
